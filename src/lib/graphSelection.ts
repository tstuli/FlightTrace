...
export function pinnedGraphChannels(model: ModelProfile, channels: ChannelDefinition[], maximum: number): string[] {
  return channels
...
export function graphSelectionForModel(model: ModelProfile, channels: ChannelDefinition[], maximum: number): string[] {
  ...
  const pinned = pinnedGraphChannels(model, channels, maximum)
